...
Geometry::Geometry() : translationMat(glm::mat4(1.0f)), scaleMat(glm::mat4(1.0f)), rotationMat(glm::mat4(1.0f)),
ambient(glm::vec3(0.2f)), diffuse(glm::vec3(0.5f)), specular(glm::vec3(1.0f)), shininess(32.0f)
{
}

Geometry::Geometry(std::string objFilename, GLfloat pointSize, glm::vec3 rotationAxis)
	: pointSize(pointSize), rotationAxis(rotationAxis),
	translationMat(glm::mat4(1.0f)), scaleMat(glm::mat4(1.0f)), rotationMat(glm::mat4(1.0f)),
	ambient(glm::vec3(0.2f)), diffuse(glm::vec3(0.5f)), specular(glm::vec3(1.0f)), shininess(32.0f)
{
...
void Geometry::parseOBJFile(std::string objFilename)
{
...
void Geometry::parseOBJFile2(std::string objFilename)
{
	...
	if (objFile.is_open())
	{
		...
		while (std::getline(objFile, line))
		{
			...
			else if (label == "f")
			{
				...
				int match1 = sscanf(i1.c_str(), "%d/%d/%d", &vertexIndex[0], &uvIndex[0], &normalIndex[0]);
				int match2 = sscanf(i2.c_str(), "%d/%d/%d", &vertexIndex[1], &uvIndex[1], &normalIndex[1]);
				int match3 = sscanf(i3.c_str(), "%d/%d/%d", &vertexIndex[2], &uvIndex[2], &normalIndex[2]);
...
void Geometry::parseOBJFile3(std::string objFilename)
{
	// Variables used to parse vertices, normals, and their indices
	std::vector<glm::vec3> vertexIndices;
	std::vector<glm::vec3> normalIndices;
...
void Geometry::update(glm::mat4 transform)
{
...
void Geometry::spin(float deg, glm::vec3 rotationAxis)
{
...
void Geometry::rotate(float deg, glm::vec3 rotationAxis)
{
...
void Geometry::rotateModel(float deg, glm::vec3 rotationAxis)
{
...
void Geometry::resetOrientation()
{
...
void Geometry::resetPosition()
{
	model = glm::inverse(translationMat) * model;
	translationMat = glm::mat4(1.0f);
}

void Geometry::scale(float scaleFactor)
{
	model = glm::scale(model, glm::vec3(scaleFactor));
	scaleMat = glm::scale(scaleMat, glm::vec3(scaleFactor));
}

void Geometry::resetScale()
{
	model = model * glm::inverse(scaleMat);
	scaleMat = glm::mat4(1.0f);
}

void Geometry::setAmbient(glm::vec3 color)
{
	ambient = color;
}

void Geometry::setDiffuse(glm::vec3 color)
{
	diffuse = color;
}

void Geometry::setSpecular(glm::vec3 color)
{
	specular = color;
}
...
GLfloat Geometry::getPointSize()
{
	return pointSize;
}

glm::vec3 Geometry::getAmbient()
{
	return ambient;
}

glm::vec3 Geometry::getDiffuse()
{
	return diffuse;
}

glm::vec3 Geometry::getSpecular()
{
	return specular;
}
